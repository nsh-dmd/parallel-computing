...
void calculate() {
    ...
    for (int i = 0; i < XSIZE; i++) {
        for (int j = rank*row_partition; j < row_partition*rank+row_partition; j++) {
            complex_t c, z, temp;
            int iter = 0;
            c.real = (xleft + step * (i));
...
int main(int argc, char **argv) {
    ...
    if(rank == 0) {
        ...
        for (int i = 1; i < nprocs; i++) {
            MPI_Recv(&pixel[row_partition*i*XSIZE], row_partition * XSIZE, MPI_INT, i, 1, MPI_COMM_WORLD, &status);
        }
        ...
        MPI_Send(&pixel[row_partition*rank*XSIZE], row_partition * XSIZE , MPI_INT, 0, 1, MPI_COMM_WORLD);
    }
